...
def generate_Report(stress_type, nro_capas, simulacion, append_mode):
    ...
    nombre_archivo = 'Reporte_' + stress_type + '_Capas_Sim' + str(simulacion) + '.rpt'
...
# size_of_square = 31 x 31 elementos
# capa  : Capa (numero) a la que se le esta calculando el promedio. int()
def generate_Report_each_element_layer(stress_type, elements_array, size_of_square, capa, simulacion, append_mode = ON):
    zz = size_of_square # Filas
    xx = size_of_square # Columnas    
    ...
    for i in range(zz): # Filas
        for k in range(xx): # Columnas
            new_name_XY_data_element = stress_type + '_' + str(elements_array[i, k]) + '_' + str(capa)
            temp_Stress = session.xyDataObjects['S:' + stress_type + ' PI: BLANK_3D-1 E: ' + str(elements_array[i, k]) + ' IP: 1']
    ...
    nombre_archivo = 'Reporte_' + stress_type + '_elementos_capas_' + str(simulacion) + '.rpt'
    session.xyReportOptions.setValues(totals=OFF, minMax=OFF)
...
avance_en_capas = 4489  # Avance en las capas hacia abajo
...
for i in range(nro_capas):
    ...
    if not isinstance(elements_array[0, 0], (int, long)):
        ...
        generate_Report_each_element_layer(stress_type, elements_array, size_of_square, capa, simulacion, append_mode = ON)
        first_element = int(first_element + avance_en_capas)
    
    
    
append_mode = OFF
generate_Report(stress_type, nro_capas, simulacion, append_mode)

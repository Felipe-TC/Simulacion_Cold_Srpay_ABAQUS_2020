...
def creating_Stress_xy_data_one_layer(odb_name, stress_type, first_element, last_element, size_of_square, avance_en_columnas, frame_pedido):
    # session.viewports['Viewport: 1'].setValues(displayedObject=o3)
    odbName=session.viewports[session.currentViewportName].odbDisplay.name
    session.odbData[odbName].setValues(activeFrames=(('Shot', (frame_pedido, )), ))
...
# size_of_square = 31 x 31 elementos
# capa  : Capa (numero) a la que se le esta calculando el promedio. int()
def generate_Report_each_element_layer(stress_type, elements_array, size_of_square, capa, simulacion, frame_pedido, append_mode = ON):
    zz = size_of_square # Filas
    xx = size_of_square # Columnas    
    ...
    if ':' in frame_pedido:
        ...
        nombre_archivo = 'CS_Reporte_' + stress_type + '_elementos_capas_frame' + str(framito) + '_Sim' + str(simulacion) + '.rpt'
    else:
        nombre_archivo = 'CS_Reporte_' + stress_type + '_elementos_capas_frame' + str(frame_pedido) + '_Sim' + str(simulacion) + '.rpt'
...
mdb_name = 'D:\Pipe\Tesis\ABAQUS_Various\CAEs\Cold_Spray_3D\Ghelichi_2014\CS_Ghelichi_2014_325_Particles_Randomized\Sim3\Sim3_Ghelichi_2014_more_Field_Output_Request_40.cae'
odb_name = 'D:\Pipe\Tesis\ABAQUS_Various\Outputs_Abaqus\Sim3_Ghelichi_2014_CS_325-nonet.odb'
...
stress_type = 'S11'     # Estress en x = S11,  o z = S33
...
for i in range(nro_capas):
    ...
    elements_array = creating_Stress_xy_data_one_layer(odb_name, stress_type, first_element, last_element, size_of_square, avance_en_columnas, frame_pedido)
    if not isinstance(elements_array[0, 0], (int, long)):
        ...
        generate_Report_each_element_layer(stress_type, elements_array, size_of_square, capa, simulacion, frame_pedido, append_mode = ON)
